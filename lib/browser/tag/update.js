 ...
 * @returns { undefined }
 */
export default function updateExpression(expr) {
  var dom = expr.dom,
    attrName = expr.attr,
    value = tmpl(expr.expr, expr.scope),
  ...
  if (expr.isRtag && value) return updateDataIs(expr, expr.scope)
  if (old === value) return
  ...
  // event handler
  if (isFunction(value)) {
    setEventHandler(attrName, value, dom, expr.scope)
  // show / hide
