...
export function getTag(dom) {
  return dom.tagName && __TAG_IMPL[getAttr(dom, 'data-is') ||
    getAttr(dom, 'is') || dom.tagName.toLowerCase()]
}
...
export function mountTo(root, tagName, opts, ctx) {
  ...
  var conf = extend({opts: opts || {}}, Tag.parse(root, impl))
  var tag = new Tag(conf, root)
...
export function moveVirtual(src, target) {
  ...
  while (el) {
    ...
    if (el === this.tail) {
      frag.appendChild(el)
      src.insertBefore(frag, target.head)
      break
    }
  }
}
